...
%       testing using my parafunc
        D_z(i) = phi0*exp(-(z(i)^2)/4)*(sigma^(1/p))*double(feval(symengine,'gamma',(1/p)))*((sqrt(2*pi)*rho*p*alpha^(1/p)*(1+beta*R0))^-1)*((1/sigma)*parafunc(-1/p,-z(i))+(gamma*beta+beta/p)*parafunc(-1/p-1,-z(i)));
    end
end